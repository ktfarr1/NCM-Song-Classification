...
if __name__ == '__main__':
    ...
    for b in [8,16,40,80,125,250]:
        ...
        rand_search = RandomizedSearchCV(clf,params,n_jobs=2,refit=True,random_state=0,n_iter=14)
        start = time()
        rand_search.fit(Xtr, Ytr)
        print "Random Search took {} for 14 candidates".format(time()-start)
    ...
    for c in np.arange(1,6):
        ...
        clf = neighbors.RadiusNeighborsClassifier(radius=150.0, metric='precomputed',p=2,outlier_label=-1)
        clf.fit(Xtr,Ytr)
        print "Score:"
